...
if generate_grid:
    ...
    generate_grid_command_run = subprocess.call(
        [sys.executable, "generate_grid.py", star]
    )
...
if run_on_grid:
    for grid_point in parameter_grid.index:
        ...
        command_run = subprocess.call(
            [sys.executable, "run_lsd.py", star, str(grid_point)]
        )
        if command_run != 0:
            break
...
plt.plot(
    ...
    "D-",
    color="blue",
    label=f"Mean {nr_of_combinations}/{nr_of_combinations} RVs",
)
plt.plot(
    mjd,
    rv_selection_mean,
    "D-",
    color="violet",
    label=f"Mean {use_n_time_series}/{nr_of_combinations} RVs",
)
